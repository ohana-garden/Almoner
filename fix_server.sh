...
echo "🛠️  Fixing Server API Integration..."

# Rewrite src/server.ts to await the new Async Engine methods
echo "📝 Updating src/server.ts..."
cat << 'YW_SERVER' > src/server.ts
/**
 * Almoner API Server
 *
...
# Commit and Push
echo "💾 Committing API Server Fix..."
git add src/server.ts
git commit -m "Fix: Updated server.ts to await new async engine methods"
git push origin main

echo "✅ Server Fix Complete! Deploying to Railway..."
